...
exports.create = async (req, res) => {
    ...
    res.render('new_company_form', {
        user: req.session.user,
        customJs: 'new_company_form',
        countries: countries
    });
};
...
exports.add = async (req, res) => {
    ...
    form.append('name', req.body.company_name);
    ...
    form.append('companyNumber', req.body.company_number);
    form.append('phoneNumber', req.body.phone_number);
    if (req.body.is_payroll == 'on') {
        form.append('isPayroll', '1');
    } else {
        form.append('isPayroll', '0');
    }
    form.append('payrollReportName', req.body.payroll_report_name);
    form.append('payrollReconciliationType', req.body.payroll_reconciliation_type);
    if (req.body.is_supporter == 'on') {
        form.append('isSupporter', '1');
    } else {
        form.append('isSupporter', '0');
    }
    if (req.body.is_patron == 'on') {
        form.append('isPatron', '1');
    } else {
        form.append('isPatron', '0');
    }
    ...
    await axios.post(process.env.API_URL + '/company/create', form, config)
        .then(async result => {
            //res.send(JSON.stringify({result: true, data: result.data}));
            res.redirect('/company/update/' + result.data.index);
        });
};
...
exports.getAll = async (req, res) => {
    ...
    const companies = await axios.get(process.env.API_URL + '/company/list', config)
        .then(async result => {
            let rows = [];
            await result.data.models.forEach(row => {
                rows.push({
                    ...
                    isPayroll: row.isPayroll ? 'Yes' : 'No',
                    isPatron: row.isPatron ? 'Yes': 'No',
                });
            });
            return rows;
        });
...
exports.getById = async (req, res) => {
    ...
    const entrant = await axios.get(process.env.API_URL + '/company/get/' + req.params.id, config)
        .then(async result => {
    ...
    res.render('edit_company_form', {
        user: req.session.user,
        customJs: 'edit_company_form',
        id: entrant.id,
        name: entrant.name,
        street1: entrant.street1,
        street2: entrant.street2,
        town: entrant.town,
        county: entrant.county,
        countryCode: entrant.countryCode,
        postcode: entrant.postcode,
        email: entrant.email,
        companyNumber: entrant.companyNumber,
        phoneNumber: entrant.phoneNumber,
        isPayroll: entrant.isPayroll,
        payrollReportName: entrant.payrollReportName,
        payrollReconciliationType: entrant.payrollReconciliationType,
        isPatron: entrant.isPatron,
        isSupporter: entrant.isSupporter,
        notes: entrant.notes,
        countries: countries
    });
};
...
exports.update = async (req, res) => {
    ...
    await axios.post(process.env.API_URL + '/company/update/' + req.body.id, form, config)
        .then(async result => {
            //res.send(JSON.stringify({result: true, data: result.data}));
